...
void CDLGLogin::OnOK() 
{
	...
	case LOGIN_IN:	
		...
		//查询数据库
		if(!MySqlIO.USER_ReadUserInfoWithName(m_user.GetBuffer(0),&CurrentUser))
		{
		...
		if(0!=strcmp(m_password.GetBuffer(0),CurrentPsw))
		{
			...
			//还原
			MySqlIO.USER_ReadUserInfoWithName(tempstr.GetBuffer(0),&CurrentUser);
			return ;
		}
	...
	case LOGIN_MODIFY:
		...
		if(!MySqlIO.USER_UpdateUserInfo(CurrentUser))
		{
