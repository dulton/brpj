...
CBSWndContainer::CBSWndContainer()
{
	...
	{
		m_nStartPlay[i] = FALSE;
		m_nEnableCarDetect[i] = FALSE;
		m_nStartRecord[i] = FALSE;
...
void CBSWndContainer::WinSetRect(CWnd *p,CRect rt)
{
	...
	{
		if(p== &(DlgMain->DlgTabVideo.DlgScreen.m_screenPannel.m_wndVideo[j]))
		{
			m_winrect[j]=rt;
			return ;
		}
...
{
	if((DlgMain->DlgTabVideo.DlgScreen.GetCurWindPlayState(screenNo)) == bFlag)
		return;
	if(bFlag)
	{
		DlgMain->DlgTabVideo.DlgNormal.StartPlay(screenNo);
	}
	else
	{
		DlgMain->DlgTabVideo.DlgNormal.StopPlay(screenNo);
	}
}
...
//////////////////////////////////////////////////
//开启识别
void CBSWndContainer::SetWindCarDetectState(int screenNo,bool bFlag)
{
	if((DlgMain->DlgTabVideo.DlgScreen.GetCarDetectState(screenNo)) == bFlag)
		return;
	if(bFlag)
	{
	...
	{
		DlgMain->DlgTabVideo.DlgNormal.CloseCarDetect(screenNo);

	}
}
BOOL CBSWndContainer::GetCarDetectState(int screenNo)
{
	BOOL bEnable = DlgMain->DlgTabVideo.DlgScreen.GetCarDetectState(screenNo);
	return bEnable;
}
...
{
	if((DlgMain->DlgTabVideo.DlgScreen.GetCurWindRecordState(screenNo)) == bFlag)
		return;
	if(bFlag)
	{
		DlgMain->DlgTabVideo.DlgNormal.OpenRecord(screenNo);
	}
	else
	{
		DlgMain->DlgTabVideo.DlgNormal.CloseRecord(screenNo);
	}
}

BOOL CBSWndContainer::GetRecordState(int screenNo)
{
	BOOL isRecord = DlgMain->DlgTabVideo.DlgScreen.GetCurWindRecordState(screenNo);
	return isRecord;
}

//////////////////////////////////////////////////
//抓拍图像
void CBSWndContainer::Capture(int screenNo)
{
	DlgMain->DlgTabVideo.DlgNormal.Capture(screenNo);
}
