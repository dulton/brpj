...
BOOL CDLGSetRecord::OnInitDialog() 
{
	...
	//添加设备列表的区域
	for(int i=0;i<DlgMain->DlgTabVideo.DlgDeviceTree.DlgAddDevice.AreaCount;i++)
	{
		if(DlgMain->DlgTabVideo.DlgDeviceTree.DlgAddDevice.AreaList[i].name != NULL)
			m_area.AddString(DlgMain->DlgTabVideo.DlgDeviceTree.DlgAddDevice.AreaList[i].name);
	}
...
void CDLGSetRecord::Struct2M(void)
{
	...
	{
		if(	camid == DlgMain->DlgTabVideo.DlgDeviceTree.iplist[i].camID)
		{
			AreaStr =DlgMain->DlgTabVideo.DlgDeviceTree.iplist[i].area ;
			CamStr =DlgMain->DlgTabVideo.DlgDeviceTree.iplist[i].name ;	
			m_camip =DlgMain->DlgTabVideo.DlgDeviceTree.iplist[i].ip ;	
			break;
		}
	}
	...
	{
		//==当前选择的区域名
		if(DlgMain->DlgTabVideo.DlgDeviceTree.iplist[i].area == AreaStr )
			m_cam.AddString(DlgMain->DlgTabVideo.DlgDeviceTree.iplist[i].name ); 
	}
	UpdateData(FALSE);
	...
	COleDateTime	StartT(data.StartYear,data.StartMon,data.StartDay,	\
					data.StartHour,data.StartMin,data.StartSec);
	m_StartMon=StartT;
	m_StartHour=StartT;

	COleDateTime	EndT(data.EndYear,data.EndMon,data.EndDay,	\
...
void CDLGSetRecord::OnCloseupComboCam() 
{
	...
	for(int i=0;i<DlgMain->DlgTabVideo.DlgDeviceTree.iptotal;i++)
	{
		//==当前选择的区域名
		if(DlgMain->DlgTabVideo.DlgDeviceTree.iplist[i].area ==AreaStr &&
			DlgMain->DlgTabVideo.DlgDeviceTree.iplist[i].name == CamStr)
		{
			m_camip=DlgMain->DlgTabVideo.DlgDeviceTree.iplist[i].ip;
			camid=DlgMain->DlgTabVideo.DlgDeviceTree.iplist[i].camID;
			break;
		}
	}
