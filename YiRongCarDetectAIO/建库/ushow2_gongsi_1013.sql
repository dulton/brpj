...
-- Created by wyx on 2014-10-13, 14:47:59 --
--------------------------------------------

spool ushow2_gongsi_1013.log
...
comment on column TB_DETECT_DEVICE.sip
  is '服务器IP';
comment on column TB_DETECT_DEVICE.curdetect
  is '当前识别个数';
comment on column TB_DETECT_DEVICE.detectlimit
  is '识别最大个数限制';
...
(
  ...
  nlevel            NUMBER,
  ndetectlimit      NUMBER,
  ncurrentdetect    NUMBER default 0
)
;
...
comment on column TB_USER.ndetectlimit
  is '识别数量限制';
comment on column TB_USER.ncurrentdetect
  is '当前识别的个数';
alter table TB_USER
